...
Suponga\-mos que \sucn{x}, \sucn{y}, \sucn{z}\ son sucesiones tales que $\,\lim\sucn{x}=\lim\sucn{z}=\alpha\,$ y existe un n\'umero natural $m_{0}$ tal que para todo $n\geqslant m_{0}$ se verifica que $x_{n}\leqslant y_{n}\leqslant z_{n}$, entonces la sucesi\'on \sucn{y}\ es convergente y $\lim\sucn{y}=\alpha$.
\end{proposicion}
...
\begin{equation}
\alpha-\eps<x_{p}<\alpha+\eps\qquad {\rm y}\qquad \alpha-\eps<z_{q}<\alpha+\eps \label{eq:1}
\end{equation}
...
$$
\alpha-\eps<x_{n}\!\leqslant y_{n}\!\leqslant z_{n}<\alpha+\eps,
$$
y, por tanto, $\alpha-\eps<y_{n}<\alpha+\eps$. Hemos probado así que $\lim\sucn{y}=\alpha$.\fin
...
letra griega $\gamma$. %No se sabe si dicho número es racional o irracional.
\begin{enumerate}[a)]
\item Deduce que
...
$$
Como $\suc{x_{2n}}$ es una sucesión parcial de \sucn{x}\ se tiene que $\suc{x_{2n}-x_n}\to\gamma-\gamma=0$.
...
Para calcular límites donde interviene la serie armónica $$H_n=\Arm$$ puede ser conveniente escribir dicha sucesión como $H_n=\log n+\gamma_n$ donde $\sucn{\gamma}\to\gamma$.
\end{estrategia}
...
\end{array}
$$
\sol a) Usaremos la estrategia \ref{est:armonica}. Pongamos $H_n=\log n+x_n$ donde $\sucn{x}\to\gamma$. Tenemos que:
$$
\begin{aligned}
